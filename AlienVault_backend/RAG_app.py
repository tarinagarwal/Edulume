...
def RAG_LLM_integration(content_list, query, chat_history):
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=f"You are a machine learning tutor specialized in machine learning. You will be given with a context of relevant information based on the user query. Your task is to answer the user's question based ONLY on the provided context. If the answer is not in the context, you must say, 'Sorry! I could not find the answer.'. Keep your answer clear, concise and educational.\n\nContext: {content_list}"),
        contents=chat_history
    )
